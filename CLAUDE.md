...
- **Run `harden_security.sql` (repo root) in Supabase SQL Editor if not already applied.** Fixes, in order: (1) profiles self-role-escalation — `profiles: update own` had no column restriction, so any logged-in user could set their own `role` to `'admin'` via a direct Supabase call; now a trigger blocks self-changes to `role`/`is_active` unless already admin; (2) `business_config` writes tiered by key — `role_perms`/`form50_payees`/`opex_defaults`/`emp_pay_history` admin-only, `emp_details` admin+co-admin, everything else (e.g. `biz_info`) unchanged/open; (3) `audit_log` reads restricted to admin; (4) `sales_daily`/`expenses` UPDATE restricted to admin/co-admin/manager (excludes `staff` — staff can still INSERT new entries, just not edit existing ones).
