...
/** พนักงานเคาน์เตอร์สะสมแต้มอย่างเดียว — หน้าแรก = /loyalty */
...
/** path ที่ loyalty_staff เข้าได้ (รวมประวัติใต้ /loyalty) */
...
// loyalty_staff ค่าเริ่มต้น = เห็นแค่สะสมแต้ม
export function allowedHrefs(role, perms) {
  if (role === 'admin') return NAV_TABS.map((t) => t.href);

  if (isLoyaltyOnlyRole(role)) {
    const rp = perms?.[role];
    if (!rp) return ['/loyalty'];
    return NAV_TABS.filter((t) => t.href === '/loyalty' && rp[t.href] !== false).map((t) => t.href);
  }
